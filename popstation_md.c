...
unsigned char data3[128] = {
    };
...
void convert(int ndiscs, char **inputs, char *output, char *title,
             char **titles, char *code, char **codes, int *complevels) {
  FILE *in, *out, *base, *t;
  ...
  for (ciso = 0; ciso < ndiscs; ciso++) {
    ...
    for (i = 0; i < (iso_positions[ciso] + header[9] + 0x100000) - offset;
         i++) {
      fputc(0, out);
    }
    ...
    if (complevels[ciso] == 0) {
      ...
      while ((x = fread(buffer2, 1, 0x9300, in)) > 0) {
        ...
        x = deflateCompress(buffer2, 0x9300, buffer, sizeof(buffer),
                            complevels[ciso]);
...
void usage(char *prog) {
  ErrorExit("Usage: %s main_title <titles> main_gamecode <gamecodes> "
            "<compressionlevels> <files>\n",
            prog);
}
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < ndiscs; i++) {
    ...
    if (argv[3 + (ndiscs * 2) + i][0] < '1' ||
        argv[3 + (ndiscs * 2) + i][0] > '9') {
  ...
  convert(ndiscs, argv + (3 + (ndiscs * 3)), "EBOOT.PBP", argv[1], argv + 2,
          argv[ndiscs + 2], argv + ndiscs + 3, complevels);
